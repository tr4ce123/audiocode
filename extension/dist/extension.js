      ...
      try {
        ...
        if (code) {
          ...
          if (editor) {
            ...
            const position = isEmptyDoc ? new vscode.Position(0, 0) : doc.lineAt(insertLine).range.end;
            let baseIndent = "";
            ...
            console.log(`\u{1F50D} detected indent (${baseIndent.length} spaces): '${baseIndent}'`);
            const snippetBody = processedCode.split("\n").map((ln, idx) => idx === 0 ? ln : baseIndent + ln).join("\n") + "\n";
